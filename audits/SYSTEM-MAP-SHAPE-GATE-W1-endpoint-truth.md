...
| E6 | **`844` vs `852` is RESOLVED — both were right.** | Python/`len()` counts **characters**; the gate's `LC_ALL=C awk length()` counts **bytes**. `system-map.md` line 451 is **852 bytes = 844 characters** (8 multi-byte glyphs). Neither figure was wrong; the instrument was simply unstated on both sides. Closed permanently by naming the instrument in §5's new line and in the gate's PASS summary, which now says *bytes*. The violation message keeps the word `chars` because R2 mandates that string verbatim. This is the measured-baseline law in miniature: a number without its instrument is not a measurement. |
