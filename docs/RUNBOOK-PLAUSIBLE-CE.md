...
- **First-party proxy (OPS-PLAUSIBLE-FIRSTPARTY-PROXY-W1, 2026-07-15):** to evade ad-block, the landing loads the script + posts events **same-origin** through 2 routes on the **apex `algovault.com` Caddy** — `handle /js/insights.js` (`rewrite * /js/pa-gfOGxVcQKNfocPr5V3vR8.js` → `reverse_proxy 127.0.0.1:8000`; cache-eligible) and `handle /pa/event` (`rewrite * /api/event` → `reverse_proxy 127.0.0.1:8000` with `header_up X-Forwarded-For {http.request.header.CF-Connecting-IP}`; never cached). The live landing tag is `<script async src="/js/insights.js">` + `plausible.init({endpoint:"/pa/event"})` (NOT `data-api` — deprecated; the site domain is baked into the pa- script). The `plausible.algovault.com` vhost stays (direct dashboard + the upstream the apex proxies to). Caddyfile backup `…bak-paproxy-<epoch>`.
...
4. Copy the **exact** snippet the site's "Installation" tab shows. The CE serves the modern **`pa-` script format** (`<script async src="https://plausible.algovault.com/js/pa-gfOGxVcQKNfocPr5V3vR8.js"></script>` + `plausible.init()`). **The LIVE landing no longer uses this direct URL** — since OPS-PLAUSIBLE-FIRSTPARTY-PROXY-W1 (2026-07-15) it loads the **first-party** form `<script async src="/js/insights.js">` + `plausible.init({endpoint:"/pa/event"})` via the apex proxy (§1) for ad-block evasion; the pa- id + `plausible.algovault.com` URL remain the CE-internal script the proxy serves.
