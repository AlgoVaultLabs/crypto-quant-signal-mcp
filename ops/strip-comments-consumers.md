...
| `scripts/check-form-action-conformance.mjs` | CANCEL-PATH-CSP-FORM-ACTION-W1 CH2 (2026-09-22) | Strips `<!-- -->` from landing/**.html before scanning `<form>` tags (a commented-out form must not count). Deliberately NOT used on TypeScript: the first cut of this gate stripped src/** and its adversarial review reproduced this module's documented string-blindness blanking live code — `'//'` or `/*` inside a string or regex opens a "comment" (measured on the real tree: src/lib/auth-providers.ts:46-47 and src/lib/channel-registry.ts:112), which silently dropped routes and forms. TS is read through the TypeScript AST instead, where comments never exist. The HTML (`angle`) family has no such hazard. |
