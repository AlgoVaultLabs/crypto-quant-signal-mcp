...
exports.getTradeSignal = getTradeSignal;
...
async function getTradeSignal(input) {
    const coin = input.coin.toUpperCase();
    const timeframe = input.timeframe || '1h';
    const includeReasoning = input.includeReasoning !== false;
    // License gate
    if (!(0, license_js_1.canAccessCoin)(coin) || !(0, license_js_1.canAccessTimeframe)(timeframe)) {
        const msg = (0, license_js_1.freeGateMessage)(coin, timeframe);
        throw new Error(msg);
    }
    ...
    const [candles, metaCtx] = await Promise.all([
        (0, hyperliquid_js_1.fetchCandles)(coin, timeframe, startTime),
        (0, hyperliquid_js_1.fetchMetaAndAssetCtxs)(),
    ]);
    ...
    // ── Compute indicators ──
    const rsiVal = (0, indicators_js_1.rsi)(closes, 14);
    const ema9Val = (0, indicators_js_1.emaLast)(closes, 9);
    const ema21Val = (0, indicators_js_1.emaLast)(closes, 21);
    // EMA crossover detection
    const ema9Series = (0, indicators_js_1.ema)(closes, 9);
    const ema21Series = (0, indicators_js_1.ema)(closes, 21);
    ...
    if (ema9Series && ema21Series && ema9Series.length >= 2) {
        ...
        if (!isNaN(curr9) && !isNaN(prev9) && !isNaN(curr21) && !isNaN(prev21)) {
            if (curr9 > curr21 && prev9 <= prev21)
                emaCross = 'BULLISH'; // golden cross
            else if (curr9 < curr21 && prev9 >= prev21)
                emaCross = 'BEARISH'; // death cross
            else if (curr9 > curr21)
                emaCross = 'BULLISH';
            else if (curr9 < curr21)
                emaCross = 'BEARISH';
        }
    ...
    let fundingScore = 0;
    if (fundingRate < 0)
        fundingScore = 60; // shorts paying longs = bullish
    else if (fundingRate > 0.0005)
        fundingScore = -60; // high positive = bearish
    // OI score (15% weight)
    let oiScore = 0;
    if (openInterest > 0) {
        if (priceChange > 0)
            oiScore = 60; // OI + price up = confirmation
        else if (priceChange < 0)
            oiScore = -40; // OI + price down = divergence
    }
    ...
    let regime = 'RANGING';
    if (emaCross === 'BULLISH' && rsiVal !== null && rsiVal < 70)
        regime = 'TRENDING_UP';
    else if (emaCross === 'BEARISH' && rsiVal !== null && rsiVal > 30)
    ...
    if (includeReasoning) {
        ...
        if (rsiVal !== null) {
            if (rsiVal < 30)
                parts.push(`RSI at ${rsiVal.toFixed(1)} suggests oversold conditions.`);
            else if (rsiVal > 70)
                parts.push(`RSI at ${rsiVal.toFixed(1)} suggests overbought conditions.`);
            else
                parts.push(`RSI at ${rsiVal.toFixed(1)} is neutral.`);
        }
        if (emaCross === 'BULLISH')
            parts.push('EMA 9 crossing above EMA 21 confirms bullish momentum.');
        else if (emaCross === 'BEARISH')
            parts.push('EMA 9 crossing below EMA 21 confirms bearish momentum.');
        if (fundingRate < 0)
            parts.push('Negative funding rate means shorts are paying longs.');
        else if (fundingRate > 0.0005)
            parts.push('High positive funding suggests crowded longs.');
    ...
    try {
        (0, performance_db_js_1.recordSignal)(coin, signal, confidence, timeframe, currentPrice);
    }
    catch {
...
//# sourceMappingURL=get-trade-signal.js.map