...
### Changed
- Clearer tool descriptions across all seven tools, so an agent picking a tool has more to go on. `get_trade_signal` now points new integrations at `get_trade_call`, and `scan_trade_calls`' `exchange` parameter says plainly that it takes one venue.

### Fixed
- The LobeHub manifest's tool descriptions were out of step with the ones the server actually serves. All seven are now generated from the same source, and a test keeps them that way.
- `/capabilities` and `tools/list` gave different descriptions for `get_trade_signal`; the alias steering hint reached only one of them. Both now project from one source.
...
### Changed
- Corrected the package description — the perp-venue coverage is now stated qualitatively so it cannot go stale as venues are added.
...
### Fixed
- Corrective republish: the npm tarball now carries the v1.22.0 "What's new" + de-enumerated coverage copy (release tag-placement fix — the v1.22.0 tag predated the doc/copy commits). No functional change from 1.22.0.
...
- Hosted outbound webhook delivery: subscribe a URL to receive `trade_call` and `regime_shift` push events. HMAC-SHA256 signed, idempotent, retried, quota-gated. Endpoints: `POST` / `GET` / `DELETE /api/webhooks`, plus `POST /api/webhooks/:id/test`. API key required.
- `GET /api/verify-signal?hash=<signal_hash>` resolves the `verify_url` carried in every payload to that call's public, on-chain-anchored record.

### Added — Platform integrations
- Two new reference implementations in [algovault-integrations](https://github.com/AlgoVaultLabs/algovault-integrations): **Hummingbot** and **FreqTrade** (7 examples total).
- Three new tiles (Cryptohopper, Hummingbot, FreqTrade) on [algovault.com/integrations](https://algovault.com/integrations).

### Internal
- Monitoring, ops, and adapter hardening. No change to the MCP tool surface (`tools/list` unchanged).
