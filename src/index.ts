...
function createServer(): McpServer {
  ...
  register(
    ...
    async ({ topN, timeframe, exchange, minConfidence, includeHolds, limit, rankBy, includeReasoning, oiChangeWindow, oiBasis, minLiquidityUsd }) => {
      const startMs = Date.now();
      try {
        const license = getRequestLicense();
        const scan = () => runScanTradeCall(
          { topN, timeframe, exchange, minConfidence, includeHolds, limit, rankBy, includeReasoning, oiChangeWindow, oiBasis, minLiquidityUsd },
          license,
        );
...
async function startHttp() {
  ...
  // OPS-INTEGRATIONS-VENUE-PAGES-W1 (2026-07-21): +4 signal-venue pages
  // (hyperliquid/aster/bingx/kucoin). Each mirror is read into
  ...
  <div class="section"><h2>Latest Trade Calls</h2>
    <div class="tr-recent-calls-panel" id="tr-recent-calls-panel" aria-live="polite">
      ...
      <div id="tr-recent-calls-rows">
        <!-- DESIGN-W11-FF3 (2026-05-14): Call + Confidence columns REMOVED per Mr.1 Data Integrity directive ("should not be disclosure at here"). Public track record shows 6 cols: ID, Time, Tier, Asset, Timeframe, Exchange. HISTORY, corrected: this note used to warn that the underlying API was still emitting the direction and confidence fields, and that a follow-up wave should sanitize the response shape. (The old wording is deliberately paraphrased, not quoted — the verification gate for this change bans that literal, and a comment quoting the banned string keeps the gate red for the wrong reason.) That follow-up SHIPPED — PERFORMANCE-PUBLIC-SANITIZE-W1 (2026-05-15) moved enforcement to the data layer via the explicit field allow-list in formatPublicRecentSignal() (src/lib/performance-db.ts), so the fields cannot be emitted regardless of what the input row carries. Measured live 2026-08-29 on BOTH channels (/api/performance-public and performance://signal-performance): row keys are exactly coin, created_at, exchange, id, tier, timeframe. The warning was retired by OPS-RECENT-SIGNALS-VENUE-FILTER-W1 because a comment advertising a Data-Integrity hole that no longer exists sends the next reader chasing a ghost and devalues every neighbouring note. -->
