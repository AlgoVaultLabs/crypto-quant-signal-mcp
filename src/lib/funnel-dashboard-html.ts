...
export function renderFunnelDashboardHtml(): string {
  ...
  const js = [
    ...
    "    fixHtml:'<div class=\"fixline\"><b>\\u2705 Fix shipped (FUNNEL-FIX-AGENT-X402-NUDGE-W1, live):</b> x402 pay-per-call surfaced in-protocol at the quota edge. Flat billing since 2026-08-08 meters every verdict, HOLD included, so agents now consume metered calls and reach the daily/monthly wall \\u2014 willingness-to-pay is the constraint, not the gate.</div>',",
    "    chLab:'By channel (retention d7 \\u2014 quality signal)',chans:ac});",
    // OPS-QUOTA-FUNNEL-WALL-SPLIT-W1 — each wall on its own terms. `unknown` is its own bucket and
    // is never folded into `monthly`; a suppressed cell shows its n and the reason, never a number.
    "  var qws=af.quota_wall_split;",
    "  if(!qws){el('wallsplit-note').textContent='Wall split unavailable \\u2014 the query failed for this window.';}else{",
    "    var wrows=qws.cells.map(function(c){",
    "      var rate=c.suppressed_reason?('\\u2014 '+c.suppressed_reason):pct(c.activated_to_wall);",
    "      return [c.bucket,f(c.sessions)+(c.low_confidence?' (n<30)':''),f(c.rows),rate];});",
    "    table(el('wallsplit'),['Wall','Sessions','Events','Activated \\u2192 wall'],wrows);",
    "    var mix=qws.mix;",
    "    var mixTxt=mix.suppressed_reason?('Wall mix suppressed \\u2014 '+esc(mix.suppressed_reason)+'.'):('Wall mix: daily '+pct(mix.daily_pct)+' \\u00b7 monthly '+pct(mix.monthly_pct)+'.');",
    "    el('wallsplit-mix').innerHTML='<b>'+mixTxt+'</b> '+esc(mix.denominator_note)+' (n='+f(mix.denominator)+', '+f(mix.excluded_unknown)+' unknown excluded).'",
    "      +' Pooled quota-crossing n='+f(qws.pooled_sessions)+(qws.pooled_low_confidence?' (n<30)':'')+'; '+f(qws.multi_bucket_sessions)+' session(s) hit BOTH walls, so the cells do not sum to the pooled stage.';",
    "    var srows=qws.stages.map(function(s){var cv=s.discriminator_cutover;",
    "      return [s.event_type,f(s.sessions.daily),f(s.sessions.monthly),f(s.sessions.unknown),",
    "        (cv.absent_until==null&&cv.live_since==null)?'\\u2014':((cv.absent_until||'?')+' \\u2192 '+(cv.live_since||'?')),",
    "        s.window_predates_cutover?'window predates cutover':'post-cutover'];});",
    "    table(el('wallsplit-stages'),['Stage','Daily','Monthly','Unknown','Discriminator cutover (bounds, UTC)','Window'],srows);",
    "    var cl=qws.claim;",
    "    el('wallsplit-claim').innerHTML='<b>Claim stage (free_key_claimed):</b> '+f(cl.claims)+' claim(s) this window'",
    "      +(cl.first_claim_at?' \\u00b7 first ever '+esc(cl.first_claim_at):'')",
    "      +' \\u00b7 inherited usage: '+(cl.inherited_usage_total==null?'\\u2014':(f(cl.inherited_usage_total)+' calls across '+f(cl.with_inherited_usage)+' claim(s), max '+f(cl.inherited_usage_max)))+'.';",
    "    var prows=qws.pending.map(function(p){return [p.question,p.blocker,p.evidence];});",
    "    table(el('wallsplit-pending'),['PENDING \\u2014 not derivable','Blocker','Evidence'],prows);",
    "    var notes=esc(qws.unit_note)+' '+esc(qws.cutover_note)+' '+esc(qws.guard_note);",
    "    if(qws.warnings&&qws.warnings.length)notes+=' \\u26a0 '+qws.warnings.map(esc).join(' \\u00b7 ');",
    ...
    "  var lc=d.lifecycle;if(lc){",
    "    table(el('lcsends'),['Step','State','Would send','Sent','Suppressed','Capped'],lc.sends_by_step.map(function(x){return [x.step,x.state,f(x.would_send),f(x.sent),f(x.suppressed),f(x.capped)];}));",
    "    el('lcsupp').textContent='Unsubscribes '+f(lc.unsubscribes)+' · bounces+complaints '+f(lc.bounces_complaints)+' · bounce rate '+(lc.bounce_rate_pct===null?'n/a (nothing sent yet)':lc.bounce_rate_pct.toFixed(1)+'%');",
    "    var ic=lc.identity_claim_rate;",
    "    table(el('lcident'),['Metric','Value'],[['Identity claim rate',pct(ic.pct)+' ('+f(ic.bound)+' bound / '+f(ic.activated)+' activated)'],['Lifecycle-attributed upgrades',lc.attributed_upgrades===null?'unavailable':f(lc.attributed_upgrades)]]);",
    "    var e=lc.elicitation;",
    "    el('lcelicit').textContent='Elicitation-capable '+(e.capable_share===null?'n/a':pct(e.capable_share))+' of '+f(e.handshaking)+' handshaking sessions'+(e.low_sample?' (n<30 — not yet decisive)':'')+' · '+f(e.no_handshake)+' sessions never sent initialize, which is NOT the same as not capable.';",
    "  }",
    ...
    "  var ca=d.client_activity_24h;if(ca){var rawTop=f(ca.calls.raw_api)+(ca.calls.raw_api_top1_pct!=null?(' (top client '+ca.calls.raw_api_top1_pct+'%)'):'');var tgb=ca.calls.tg_bot_breakdown;",
    "    table(el('client'),['Client type','Calls (24h)','Sessions (24h)'],[['Total',f(ca.calls.total),f(ca.sessions.total)],['\\ud83d\\udfe2 Recognized',f(ca.calls.recognized),f(ca.sessions.recognized)],['\\ud83d\\udd0c Raw API',rawTop,f(ca.sessions.raw_api)],['\\ud83d\\udcb3 Paid',f(ca.calls.paid),f(ca.sessions.paid)],['\\u00a0\\u00a0\\u21b3 subscription',f(ca.calls.paid_subscription),f(ca.sessions.paid_subscription)],['\\u00a0\\u00a0\\u21b3 x402/a2mcp',f(ca.calls.paid_x402),f(ca.sessions.paid_x402)],['\\ud83d\\udd01 TG bot',f(ca.calls.tg_bot)+' (W'+f(tgb.watch)+'\\u00b7SW'+f(tgb.scanwatch)+'\\u00b7S'+f(tgb.scan)+')',f(ca.sessions.tg_bot_subscribers)+' subs']]);}",
  ...
  const body = [
    ...
    '<div class="muted" id="wallsplit-claim"></div>',
    '<div id="wallsplit-pending"></div>',
    '<div class="muted" id="wallsplit-note"></div>',
    ...
    '<div class="st">Agent engagement + HOLD volume <span style="font-size:11px;font-weight:400;color:#8b949e;float:right">~99% of external calls are HOLD verdicts — metered as calls since 2026-08-08</span></div>',
    ...
    '<div class="st">Lifecycle sends <span style="font-size:11px;font-weight:400;color:#8b949e;float:right">shipped dark · each step lights itself on its own day-7 evidence</span></div>',
    '<div id="lcsends"></div><div class="muted" id="lcsupp"></div>',
    '<div class="st">Identity claim rate &amp; elicitation</div>',
    '<div id="lcident"></div><div class="muted" id="lcelicit"></div>',
