...
async function edgexGet<T>(path: string, params?: Record<string, string | number>, retries = MAX_RETRIES): Promise<T> {
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        throw new UpstreamRateLimitError('edgeX', Number.isFinite(seconds) ? seconds : null);
      }

      if (!res.ok) {
        throw new Error(`edgeX API ${res.status}: ${res.statusText}`);
      }
  ...
  throw new Error('edgeX API: max retries exceeded');
}
