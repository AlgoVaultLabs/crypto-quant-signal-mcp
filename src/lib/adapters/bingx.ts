...
async function bingxGet<T>(path: string, params?: Record<string, string | number>, retries = MAX_RETRIES): Promise<T> {
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        throw new UpstreamRateLimitError('BingX', Number.isFinite(seconds) ? seconds : null);
      }

      if (!res.ok) {
        throw new Error(`BingX API ${res.status}: ${res.statusText}`);
      }
  ...
  throw new Error('BingX API: max retries exceeded');
}
